...
@Component({
  selector: 'app-retenue-clients',
  standalone: true,
  imports: [CommonModule, RouterModule, FormsModule, MatIconModule, MatProgressSpinnerModule, DecimalPipe],
  templateUrl: './retenue-clients.html',
  styleUrls: ['./retenue-clients.scss'],
})
export class ReportsReportsRetenueClients implements OnInit {
  ...
  load(): void {
    ...
    this.http.get<any>(`${environment.baseUrl}/rapports/ventes/retenue-clients`, { params }).subscribe({
      next: d => { this.lignes = d.lignes ?? []; this.loading = false; },
  ...
  get totalBrut()   { return this.lignes.reduce((s,l) => s + (l.montantBrut  ||0), 0); }
  get totalRetenu() { return this.lignes.reduce((s,l) => s + (l.montantRetenu||0), 0); }
  get totalNet()    { return this.lignes.reduce((s,l) => s + (l.netAPayer    ||0), 0); }
}